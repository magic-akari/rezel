...
Rust 1.96.0 parser and typed concrete syntax for Rezel.
...
parser conflicts or a clearer typed interface. The maintained Rust grammar
covers let-else statements, let chains, `if let` match guards, async and const
...
The parser does not yet claim complete Rust 1.96.0 language coverage. The
...
1.96.0 compiler in Edition 2024 mode, and concrete syntax trees are compared
...
`ac68faa20c58cbccd01ee7208bf3b6e93a7d7f96` (tag `1.96.0`).
...
`rust-src` standard-library `library` tree from Rust 1.96.0 revision
`ac68faa20c58cbccd01ee7208bf3b6e93a7d7f96`. The pinned inventory contains
1,972 files, has no exclusions, uses the complete `SourceFile` entry point in
...
It establishes practical coverage over the Rust 1.96 standard library,
