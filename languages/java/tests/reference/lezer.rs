...
const CASE_SCHEMA: &str = "rezel.lezer-java-reference-cases.v1";
const SNAPSHOT_SCHEMA: &str = "rezel.lezer-java-reference-snapshot.v1";
const CASES: &str = include_str!("../../../../tools/references/lezer/cases/java.json");
const SNAPSHOT: &str = include_str!("../../../../tools/references/lezer/snapshots/java.json");
...
#[test]
fn valid_java_csts_match_strict_and_recovering_lezer_parses() {
...
#[test]
fn invalid_java_csts_match_deterministic_lezer_recovery() {
    let (cases, snapshot) = reference_data();
...
fn parse_case(case: &ReferenceCase, strict: bool) -> Result<Tree, ParseError> {
    let mut parser = rezel_lang_java::parser().with_strict(strict);
    if let Some(top) = case.top.as_deref() {
        parser = parser.with_top(top)?;
    }
    parser.parse(&case.source)
}
