...
static EMPTY_STATE_DATA: [u16; 2] = [SequenceCode::End.raw(), SequenceCode::End.raw()];

const fn language_with_goto(goto: &'static [u16]) -> Language {
    Language {
        states: &[0, 0, 0, 0, 0, 0],
...
static TRUNCATED_GOTO: Language = language_with_goto(&[1, 2, 0]);
static UNKNOWN_GOTO_TARGET: Language = language_with_goto(&[1, 2, 3, 1, 0]);
static UNKNOWN_GOTO_SOURCE: Language = language_with_goto(&[1, 2, 3, 0, 1]);
